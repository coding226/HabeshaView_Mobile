...
//    print(accessTokenResponse.body);
    if(accessTokenResponse.statusCode == 200){
      var route = MaterialPageRoute(builder:(BuildContext context) => new LoadingPage(isSelected: true, useremail: email, userpass: password));
...
//  Register button
  Widget registerButton() {
    return ListTile(
        ...
        child: new Text("Register", style: TextStyle(fontSize: 16)),
        onPressed: () {
  ...
  Widget mobileLogin() {
    return ListTile(
        title: MaterialButton(
            height: 50.0,
            color: Color.fromRGBO(43, 49, 55, 1.0),
            textColor: Colors.black,
            child: new Text("Login with Mobile", style: TextStyle(color: Colors.white),),
            onPressed: () {
              var router = new MaterialPageRoute(
                  builder: (BuildContext context) => new SignUpForm());
              Navigator.of(context).push(router);
...
//  Setting background design of login button
  Widget loginButton() {
    return MaterialButton(
        ...
        child: new Text("Sign In", style: TextStyle(fontSize: 16),),
        onPressed: () {
          var router = new MaterialPageRoute(
...
// Facebook login
  ...
  Widget html() {
    return Html(
      ...
      """</div>""" :
      """<div><b>${loginConfigData['copyright']}
      </b></div>""",
      ...
      customTextStyle: (dom.Node node, TextStyle baseStyle) {
        if (node is dom.Element) {
          switch (node.localName) {
            case "p":
              return baseStyle.merge(TextStyle(height: 1.3, fontSize: 15));
          }
        }
        return baseStyle;
      },
    ...
    return Container(
      child: new Align(
          ...
          child: Container(
            ...
            child: loginConfigData['copyright'] == null ? Text(" ") :
            Text("${loginConfigData['copyright']}",
              textAlign: TextAlign.center,),
          ),
...
*/
          ...
          child: new Image.network(
            ...
            scale: 1.5, width: 150.0, height: 150.0,
          ),
        ),
        SizedBox(
          height: 20.0,
